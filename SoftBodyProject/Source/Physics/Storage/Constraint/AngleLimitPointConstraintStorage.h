...
class AngleLimitPointConstraintStorage
{
public:
	MD_STORAGE_READ_ONLY_COLUMN(ConstraintID, ID, ids);
	MD_STORAGE_READ_WRITE_COLUMN(AngleLimitPointConstraint, Constraint, constraints);
public:
	// 追加関数
	void Add(ConstraintID _id, AngleLimitPointConstraint&& _constraint)
	{
		ids.push_back(_id);
